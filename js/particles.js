...
(function () {
  const isMobile = window.matchMedia('(max-width: 680px)').matches || ('ontouchstart' in window && window.innerWidth < 900);

  const canvas = document.createElement('canvas');
  canvas.id = 'particles-canvas';
  Object.assign(canvas.style, {
    position: 'fixed', inset: '0', width: '100%', height: '100%',
    zIndex: '0', pointerEvents: 'none', opacity: isMobile ? '0.25' : '0.45'
  });
  ...
  function draw(ts) {
    ...
    // pause when tab is hidden or cap to FPS_CAP
